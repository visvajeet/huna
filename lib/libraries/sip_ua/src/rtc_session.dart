...
class RTCSession extends EventManager {
  ...
  void startScreenShare() async {
    ...
    var displayStream = await navigator.getDisplayMedia(mediaConstraints);
    await this._localMediaStream.dispose();
    this._localMediaStream = null;
    this.connection.getLocalStreams().clear();
    _localMediaStream = displayStream;
    this.connection.addStream(displayStream);
    try {
      await this.emit(
          EventStream(
              session: this, originator: 'local', stream: _localMediaStream));
    } catch (error) {
      print("EEEEEEEEEEEEEEEEEEEEEEEEEROOROR");
      print(error);
    }
    await renegotiate(mediaConstraints);
  }
}
